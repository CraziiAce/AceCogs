    ...
    async def stock(self, ctx, *, stock_ticker:str):
        ...
        if token.get("token") is None:
            ...
            async with aiohttp.ClientSession() as session:
                ...
                percentage_change_a = response['quote']['symbol']['latestPrice'] / response['quote']['open']
                ...
                embed = discord.Embed(
                    title = f"Stock Data for {response['quote']['symbol']['companyName']}",
                    color = embedColor,
                )
                if percentage_gain:
                    embed.add_field(name='Prices', value=f"Open: ${response['quote']['symbol']['open']}\nHigh: ${response['quote']['symbol']['high']}\nLow: ${response['quote']['symbol']['low']}\nCurrent: ${response['quote']['symbol']['latestPrice']}\nPercentage Gain: <:up_arrow:736390019136356442> %{percentage_change_final}")
